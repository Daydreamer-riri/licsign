...
export async function createBatch(db: D1Database, issuerId: string, actorId: string, body: unknown) {
    ...
    db
      .prepare(
        `INSERT INTO license_batches
          (id, issuer_id, product_id, batch_name, code_prefix, quantity, max_devices, expires_at, notes, created_by_api_key_id, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .bind(
        batchId,
